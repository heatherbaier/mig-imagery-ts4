...
resnet18 = models.resnet50(pretrained = True)#.to(device)
num_ftrs = resnet18.fc.in_features
resnet18.fc = torch.nn.Linear(num_ftrs + 2331 + 2, 1)
model = torch.nn.DataParallel(resnet50_mod(resnet18)).to(device)
criterion = torch.nn.MSELoss(reduction = 'mean')
optimizer = torch.optim.Adam(model.parameters(), lr = lr)


model_wts, val_losses_plot = train_model(model, train_dl, val_dl, criterion, optimizer, epochs, BATCH_SIZE, lr, device)
...
torch.save({
            ...
            'loss': criterion,
        }, "./trained_models/model_wl_v4_alldata_new_50epochs_r50.torch")
...
val_preds.to_csv("./val_preds_new_50epochs_r50.csv", index = False)
train_preds.to_csv("./train_preds_new_50epochs_r50.csv", index = False)
...
plt.savefig("./avg_val_preds_50epochs_r50.png")
plt.clf()
...
plt.title("Training")
plt.savefig("./avg_train_preds_50epochs_r50.png")
plt.clf()
