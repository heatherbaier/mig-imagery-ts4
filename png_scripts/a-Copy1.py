...
IMAGERY_DIR = "MEXimagery/2010/"
FULL_DIR = os.path.join(os.getcwd(), IMAGERY_DIR)
TEMP_DIR = os.path.join(os.getcwd(), "temp")

for month in os.listdir(FULL_DIR):
    
    print("Month: ", month)
    
    CUR_DIR = os.path.join(FULL_DIR, month)
    dir_length = len(os.listdir(CUR_DIR))
    num = 0
    
    for zipfolder in os.listdir(CUR_DIR):
        
        print("Image ", str(num), " of ", str(dir_length), "---- Month: ", month)
        num += 1

        image_name = zipfolder.split(".")[0]

        # Extract the RGB TIFF files into the temporary directory
        with zipfile.ZipFile(os.path.join(CUR_DIR, zipfolder), 'r') as zip_ref:
        ...
        PIL_image.save(os.path.join(os.getcwd(), "model_imagery", (image_name + ".png")))
